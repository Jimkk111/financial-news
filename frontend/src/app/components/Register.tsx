...
import { register, type RegisterRequest, type UserInfo } from '@/services/userService';

interface RegisterProps {
  onRegister: (username: string, email?: string, userId?: string, userInfo?: any) => void;
  onSwitchToLogin: () => void;
  onTabChange: (tab: string) => void;
  onUserClick: () => void;
}

export function Register({ onRegister, onSwitchToLogin, onTabChange, onUserClick }: RegisterProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        onRegister(userData.username, userData.email, userData.id.toString(), userData);
      } else {
        // 注册失败
      ...
      <main className="flex-1 pt-20 pb-6 px-4 overflow-y-auto">
        <div className="max-w-md mx-auto">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-gray-600 text-sm">加入AI金融快讯，开启智能投资之旅</p>
          </div>
              ...
              <div>
                <label className="flex items-start cursor-pointer group">
                  <div className="relative flex items-center justify-center w-5 h-5 mt-0.5">
                    ...
                    />
                    {agreedToTerms && (
                      <Check className="absolute text-white pointer-events-none" size={14} />
                    )}
                  </div>
                  <span className="ml-2 text-sm text-gray-600">
                    我已阅读并同意 
                    <a href="#" className="text-blue-600 hover:text-blue-700 font-medium"> 用户协议 </a>
                    和
